...
select_analysis_scope() {
    ...
    echo "5) 只跑 Stage 5: All SNP Site Map 建立所有SNP位點圖"
    echo "6) 只跑 Stage 6: LD Pruned SNP Site Map 建立on-linkage位點圖"
    echo "7) 只跑 Stage 7: Final SNP Calling (with LD pruning / skip LD pruning)"
    echo "8) 只跑 Stage 8: Structure Auto Generator多執行緒 STRUCTURE 分析"
