...
select_analysis_scope() {
    ...
    echo "  請先選擇要做哪種分析"
    echo "======================================================="
    echo "1) 只跑 Stage 1: Trimming (QC) & Remove Adapters"
    echo "2) 只跑 Stage 2: 比對read到參考基因組(BWA)"
    echo "3) 只跑 Stage 3: 分析 Mapping 結果並移除 Outlier (PCA)"
    echo "4) 只跑 Stage 4: 檢查潛在的 Clone 樣本並移除 (hclust)"
    echo "5) 只跑 Stage 5: All SNP Site Map 建立所有SNP位點圖(angsd)"
    echo "6) 只跑 Stage 6: LD Pruned SNP Site Map 建立no-linkage位點圖(ngsLD)"
    echo "7) 只跑 Stage 7: SNP Calling"
    echo "8) 只跑 Stage 8: Structure Auto Generator 多執行緒 STRUCTURE 分析"
